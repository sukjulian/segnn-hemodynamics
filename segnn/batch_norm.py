# Copied and adapted from "Euclidean neural networks (e3nn)" (https://github.com/e3nn/e3nn)
import torch
...
from torch_scatter import scatter  # [ADDED] scatter function for batch instance separation
...
class BatchNorm(nn.Module):
    ...
    def forward(self, input, batch_id_tensor=None):  # [CHANGED] include batch ID tensor
        ...
        if batch_id_tensor is None: batch_id_tensor = torch.zeros(input.size(0), dtype=torch.int64, device=input.device)
        # [ADDED] ^^^
        ...
        for mul, ir in self.irreps:
            ...
            if self.training or self.instance:
                ...
                # [ADDED] batched instance norm
                    ...
                    field_norm = scatter(field_norm, batch_id_tensor, dim=0, reduce='mean')[batch_id_tensor]  # [batch, mul]
                # [ADDED] ^^^
            else:
