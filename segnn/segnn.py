...
# from e3nn.nn import BatchNorm  # [CHANGED] re-implemented
import numpy as np
from e3nn.o3 import Irreps

from .o3_building_blocks import O3TensorProduct, O3TensorProductSwishGate
from .batch_norm import BatchNorm  # [CHANGED] re-named
...
class SEGNN(nn.Module):
    ...
    def __init__(
        ...
        # [ADDED] count trainable parameters
    ...
    def catch_isolated_nodes(self, graph):
        ...
        if (
            graph.has_isolated_nodes()  # [CHANGED] deprecated signature
    ...
    def forward(self, graph):
        ...
        for layer in self.layers:
            x = layer(
                x, edge_index.long(), edge_attr, node_attr, batch, additional_message_features  # [ADDED] cast to long
            )
...
class SEGNNLayer(MessagePassing):
    ...
    def __init__(
        ...
        self.update_layer_2 = O3TensorProduct(
            hidden_irreps, output_irreps, node_attr_irreps  # [CHANGED] mind output irreps
        )

        self.dim_adapt_layer = O3TensorProduct(input_irreps, output_irreps, node_attr_irreps)  # [ADDED] mind output irreps
    ...
    def setup_normalisation(self, norm):
        ...
        if norm == "batch":
            ...
            self.feature_norm = BatchNorm(self.hidden_irreps, instance=True)  # [CHANGED] batched instance norm
            self.message_norm = BatchNorm(self.hidden_irreps, instance=True)  # [ADDED] batched instance norm for message
    ...
    def forward(
        ...
        x = self.propagate(
            ...
            # [ADDED] batch ID tensor for batched instance norm
            batch=batch.view(-1, 1) if batch is not None else torch.zeros((x.size(0), 1), dtype=torch.int64, device=x.device)
            # [ADDED] ^^^
        )
        # Normalise features
    ...
    def message(self, x_i, x_j, edge_attr, additional_message_features, batch_i):  # [ADDED] mapped batch ID tensor
        """Create messages"""
        ...
        if self.message_norm:
            # [ADDED] batched instance norm for message
            if self.norm == 'batch':
                message = self.message_norm(message)
            elif self.norm == 'instance':
                message = self.message_norm(message, batch_i.squeeze())
            # [ADDED] ^^^
        return message
    ...
    def update(self, message, x, node_attr):
        ...
        x = self.dim_adapt_layer(x, node_attr) + update  # [ADDED] adapt dimensions
        return x
